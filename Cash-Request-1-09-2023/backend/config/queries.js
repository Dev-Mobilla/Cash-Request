...
const getUserByEmail = "SELECT bu.ResourceID, bu.BranchCode, bu.ZoneCode, bu.fullname,su.EmailAddress, su.RoleID FROM sysuseraccounts su INNER JOIN branchusers bu ON bu.ResourceID = su.ResourceID WHERE su.EmailAddress = ? UNION SELECT abu.ResourceID, abu.BranchCode, abu.ZoneCode, abu.fullname,asu.EmailAddress, asu.RoleID FROM adminsysuseraccounts asu INNER JOIN adminbranchusers abu ON abu.ResourceID = asu.ResourceID WHERE asu.EmailAddress = ?"

//BRANCH_REQUEST
const postRequest = "INSERT INTO cash_advance_request (idNumber, author,jobTitle, branch,area,region,zonecode,purpose,controlNo,date,travelDate,departureDate,arrivalDate,amount,area_approver,am_status,am_date,regional_approver,rm_status,rm_date,ram_approver,ram_status, ram_date,ass_vpo_approver,ass_status,ass_date,vpo_approver,vpo_status,vpo_date,request_status) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
...
const postRfRequest = "INSERT INTO revolving_fund_request (rfDate, idNumber,requestor,baseBranch, region,zonecode,period,controlNo,rfAllowance,pendingRf,totalExpenses,cashOnHand,transportation,officeSupplies,meals,others,purpose,am_approver,am_status,am_date,rm_approver,rm_status,rm_date,ram_approver,ram_status,ram_date,ass_vpo_approver,ass_status,ass_date,vpo_approver,vpo_status,vpo_date,request_status) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
...
const getAllRequestsCa = "SELECT date, controlNo, amount, region, branch, author, request_status AS status FROM cash_advance_request WHERE idNumber = ? AND request_status = ?";
const getAllRequestsRf = "SELECT rfDate AS `date`, controlNo, totalExpenses AS total, cashOnHand , region, baseBranch AS branch, requestor, request_status AS `status` FROM revolving_fund_request WHERE idNumber = ? AND request_status = ?";
